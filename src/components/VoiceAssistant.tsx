        ...
        clientTools: {
          ...
          addItemToCart: async ({ productName, quantity = 1 }) => {
            ...
            try {
              ...
              const response = { 
                success: true, 
                message: `Added ${validQuantity} ${product.name} to your cart for ₹${(product.price * validQuantity).toFixed(2)}${confidenceNote}.`,
          ...
          removeItemFromCart: async ({ productName }) => {
            ...
            try {
              ...
              const response = { 
                success: true, 
                message: `Removed ${cartItem.product?.name} from your cart${confidenceNote}.`,
          ...
          updateCartItemQuantity: async ({ productName, quantity }) => {
            ...
            try {
              ...
              if (validQuantity === 0) {
                ...
                const response = {
                  success: true,
                  message: `Removed ${cartItem.product?.name} from your cart (quantity set to 0).`,
                  product: cartItem.product?.name,
                  quantity: 0,
                  matchConfidence: matchResult.confidence
                }
              ...
              const response = { 
                success: true, 
                message: `Updated ${cartItem.product?.name} quantity to ${validQuantity}${confidenceNote}.`,
                product: cartItem.product?.name,
                quantity: validQuantity,
          ...
          placeOrder: async () => {
            ...
            try {
              ...
              const response = { 
                success: true, 
                message: `Order placed successfully! Your order of ${itemCount} items totaling ₹${orderTotal.toFixed(2)} will be delivered to your saved address. You'll receive a confirmation shortly.`,
                orderTotal,
                itemCount,
                customerName,
